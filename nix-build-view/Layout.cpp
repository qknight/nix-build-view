...
void
Layout::addWidget(Widget* w, unsigned int heightHint)
{
...
RasterizedLayout
Layout::rasterize(int width, int height)
{
    RasterizedLayout r;
    int rowsUsed = 0;
    ...
    if (rowsUsed < height) {
        for (unsigned int i = 0; i < r.m_fixedWidgets.size(); ++i)
        {
            ...
            if (type == WidgetName::BuildWidgetManager ||
                    type == WidgetName::FetchWidgetManager)
            {
                int t           = 0;
                unsigned int hH = 0;
                ...
                // use height given by widget unless the layout says it must
                // be less
                ...
                // if t now exceeds the available height, we need to limit it
                // to what is still available marry x-mess code
