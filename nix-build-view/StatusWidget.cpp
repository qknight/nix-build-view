...
int
StatusWidget::type() const
{
    return WidgetName::StatusWidget;
}

AdvancedStringContainer
StatusWidget::render(unsigned int width, unsigned int height)
{
    ...
    if (m_fetches) {
        ...
        auto str = (m_fetches == 1 ? " fetch" : " fetches");
        s2 << AdvancedString(str, COLOR_GREEN);
    }
...
void
StatusWidget::setFocus(int focus)
{
    m_focus = focus;
    update();
}
...
void
StatusWidget::setBuilds(unsigned int builds)
{
    m_builds = builds;
    update();
}
...
void
StatusWidget::setFetches(unsigned int fetches)
{
    m_fetches = fetches;
    update();
}
