...
bool BuildWidgetManagerSort(const Widget* a, const  Widget* b) {
    if ((a->type() == WidgetName::BuildWidget) && (b->type() == WidgetName::BuildWidget)) {
    ...
    for(unsigned int i=0; i < m_widgets.size(); ++i) {
        ...
        if (v->m_name == UUID) {
            ...
            if (v->m_currentPhase ==  v->m_phases.size()) {
                ...
                v << "Build of " << AdvancedString(UUID, COLOR_MAGENTA) << " completed\n";
